...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {    
		...
		StworzPiramide(gl2,1,9);
	} // end display()
	public void StworzPiramide(GL2 gl2,double size,int n)
	{
		float[] X = new float[n]; // Wspó³rzêdne wierzcho³ków na osi X
		float[] Y = new float[n]; // Wspó³rzêdne wierzcho³ków na osi Y
		...
		for (int i = 0; i < X.length; i++) {
			...
			gl2.glMaterialfv(GL2.GL_FRONT,GL2.GL_AMBIENT_AND_DIFFUSE, materials[i], 0);
			gl2.glVertex3f(X[i], 0, Y[i]);// stworzenie punktu
			gl2.glVertex3f(X[(i + 1) % n], 0, Y[(i + 1) % n]);			
			gl2.glVertex3f(0, 7, 0);
			gl2.glEnd();
		}
	}
	...
	public void init(GLAutoDrawable drawable) {
		...
		float[] swiatlo = {1.7f,1.7f,1.7f,1};
		 gl2.glEnable(GL2.GL_DEPTH_TEST);
	        gl2.glEnable(GL2.GL_NORMALIZE);
	        gl2.glEnable(GL2.GL_LIGHTING);
	        gl2.glEnable(GL2.GL_LIGHT0);
	gl2.glLightModelfv(GL2.GL_LIGHT_MODEL_AMBIENT, swiatlo, 1);
	}
